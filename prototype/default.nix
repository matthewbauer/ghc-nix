...
runCommand
  ...
  MESSAGE_CA=$( nix --experimental-features nix-command make-content-addressable --json "$MESSAGE_ORIG" | jq -r '.rewrites."'"$MESSAGE_ORIG"'"' )
  rm -r src

  mkdir src
  rsync --quiet -avz "${ ../test-project }/Main.hs" ./src
  MAIN_ORIG=$( nix-build -E 'import ${ ./compile-hs.nix } ./src "Main.hs" [ '"$MESSAGE_CA"' ]' )
  MAIN_CA=$( nix --experimental-features nix-command make-content-addressable --json "$MAIN_ORIG" | jq -r '.rewrites."'"$MAIN_ORIG"'"' )
  rm -r src
